...
| --- | --- | --- | --- |
| **김선민** | 데이터 수집·생성·적재 및 스키마 설계 | https://github.com/nimnusmik | 팀 일정 관리, TMDB 데이터 크롤링, 가상 유저 생성 및 시청 로그 생성, DB/S3 적재, 데이터 전처리 및 표준 스키마 설계, PostgreSQL 적재, AWS S3 백업(.parquet 변환), SQL RDBMS 설계/관리, 데이터 처리 파이프라인 스크립트화, 서버1 전체 Logger 기능 설계·구현, PPT 문서 제작 |
...
- 팀원 개인 회고
    - **김선민**: 깃 협업과 개발 경험이 부족한 상태에서 LLM에 의지해 새로운 기술을 적용하는 것은, 마치 한쪽 다리를 잃은 채 걷는 것과 같았다. LLM과 새로운 툴은 정말 매력적이지만, 도입 전에는 현실적인 검토를 여러 번 거쳐야 한다는 결심을 했으며, 트러블슈팅과 협업의 과정을 통해 정말 많은 것을 배운 소중한 프로젝트였다. 그리고 열심히 함께해 준 팀원분들께 진심으로 감사드린다.
    - **김장원**: 처음엔 기능 하나하나를 연결하는 것조차 조심스러웠지만, 데이터 수집부터 모델 서빙까지 전 과정을 직접 설계하고 구현하며 MLOps라는 거대한 퍼즐을 제 방식대로 맞춰볼 수 있었습니다. 도커, FastAPI, S3, PostgreSQL 같은 도구들이 처음엔 낯설었지만, 그만큼 시행착오 속에서 정말 많이 배운 시간이었습니다.
    - **최현화**: 이번 프로젝트에서 서버2의 자동화 파트와 최종 병합·공통 인프라 구축을 맡으면서, MLOps의 실전적인 자동화 경험을 제대로 쌓을 수 있었습니다. Airflow로 전체 파이프라인을 자동화하며 실무에서 에러 처리와 장애 복구의 중요성을 직접 체감했습니다. 초반에는 생소한 도구와 구조에 시행착오도 많았지만, 매일 조금씩 문제를 해결하면서 결국 모든 파이프라인이 안정적으로 돌아가는 결과를 만들어낼 수 있었던 점이 가장 보람찼습니다.
    이번 경험을 바탕으로 앞으로 더 복잡한 자동화, 더 실전적인 MLOps 환경에도 도전해보고 싶다는 생각이 들었습니다.
    - **장윤정**: 처음 접하는 도구들이 많아 어려움이 있었지만, 팀원들과 매일 스크럼 회의를 통해 함께 고민하고 배워나가는 과정이 즐거웠습니다. 각각의 도구들이 어떻게 작동하고 서로 연동되는지 잘 모르는 상태에서 시작해 시행착오도 많았지만, 매일 함께 문제를 해결해 나가면서 결국 만족스러운 결과물을 완성할 수 있어서 좋았습니다.
